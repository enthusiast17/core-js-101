...
function getPolynom(...args) {
  return (x) => {
    if (args.length === 3) return (args[0] * (x ** 2)) + (x * args[1]) + args[2];
...
function logger(func, logFunc) {
  return (...value) => {
    const start = JSON.stringify(value);
    logFunc(`${func.name}(${start.slice(1, start.length - 1)}) starts`);
    const res = func(...value);
    logFunc(`${func.name}(${start.slice(1, start.length - 1)}) ends`);
    return res;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  const state = { startFrom };
  return () => {
    state.startFrom += 1;
    return state.startFrom - 1;
  };
}
