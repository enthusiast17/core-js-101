...
function chainPromises(array, action) {
  return array.reduce((res, promise) => res.catch(() => {}).then((resResult) => (
    promise.catch(() => {}).then((promiseResult) => ([...resResult, promiseResult])))),
  Promise.resolve([])).then((arr) => arr.filter((el) => el !== undefined).reduce(action));
}
